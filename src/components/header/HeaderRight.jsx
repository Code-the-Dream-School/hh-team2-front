...
//   // Get profile and user data from Redux store
//   const { user } = useSelector((state) => state.auth); // User data from auth slice
//   const { first_name, profile_picture } = useSelector((state) => state.profile); // Profile data from profile slice
...
//   const userFirstName = first_name || user?.first_name || "Guest"; // Default to "Guest" if no name
//   const userProfilePic = profile_picture || user?.profile_picture || "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973461_640.png"; // Fallback image
...
import { HiOutlineUser, HiOutlineLogout } from "react-icons/hi"; // Import the icons
import { authActions } from "../../redux/slices/authSlice"; // Import authActions for dispatching logout action
import { toast } from "react-toastify";  // Assuming you are using this for notifications

const HeaderRight = ({ toggle, setToggle }) => {
  const [dropdown, setDropdown] = useState(false);  // Add state for dropdown visibility
  ...
  return (
    ...
    <div
      ...
      onClick={() => setToggle((prev) => !prev)} // Toggle menu visibility
    >
      {/* Conditionally render hamburger icon or close icon */}
      {toggle ? (
        <XMarkIcon className="h-6 w-6 text-gray-700" />
      ) : (
        <Bars3Icon className="h-6 w-6 text-gray-700" />
      )}
      {/* Menu text next to the icon */}
      <span className="ml-2 text-gray-700 font-semibold">Menu</span>
    </div>
  
    {/* User profile and logout button (only visible when logged in) */}
    {user ? (
      <div className="relative flex items-center space-x-2 justify-start">
        {/* Wrap the username and profile picture in a div to prevent shifting */}
        <div className="flex items-center space-x-2">
          <span
            onClick={() => setDropdown(!dropdown)} // Toggle the dropdown visibility
            className="cursor-pointer text-sm font-semibold text-gray-700 hover:text-[#e67e08]"
          >
            {userFirstName}
          </span>
        ...
        {dropdown && (
          <div
            ...
            style={{
              top: "80%", // Position dropdown directly below the profile name
              left: "-80px", // Keep the dropdown in the same position
              marginTop: "5px",
            }}
          >
            {/* Profile link */}
            ...
            {/* Logout link */}
            <div
              onClick={logout}  // Logout on click
              className="px-4 py-2 text-gray-700 hover:bg-gray-100 cursor-pointer transition duration-300 flex items-center space-x-2"
            >
              <HiOutlineLogout className="w-5 h-5 text-gray-600" />
              <span>Logout</span>
      ...
      // Show login/register buttons for screens larger than 428px
